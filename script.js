(function () {
  ...
  window.addEventListener("keydown", (e) => {
    //prevents the user from closing the dialog modal
    if (e.key == "Escape") {
      e.preventDefault();
    }
  });
  ...
  function checkEventTarget(e) {
    // checks if event triggers inside an actual square
    return e.target.attributes[0].name == "data-key";
  }

  function cleanGameBoard() {
    //removes markers from the squares
...
let gameboard = (function () {
  ...
  function isSquareEmpty(index) {
    // checks if there's already a marker in the square
  ...
  function checkDiagonals() {
    //check for 3 in a row in board diagonals
  ...
  function checkColumns() {
    //check for 3 in a row in board columns
    for (let i = 0; i < 3; i++) {
  ...
  function checkRows() {
    //check for 3 in a row in board rows
    for (let i = 0; i < 9; i += 3) {
      let firstSquare = board[i];
